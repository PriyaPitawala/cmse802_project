...
def display_image(image: np.ndarray, title="Image", scale_length_pixels=None, scale_text=None):
    """
    Displays the image with a scale bar if specified.
    
    Parameters:
    - image (np.ndarray): The image to display.
    - title (str): Title of the displayed image.
    - scale_length_pixels (int, optional): Length of the scale bar in pixels.
    - scale_text (str, optional): Text to display for the scale bar.
    """
    ...
    if scale_length_pixels is not None and scale_text is not None:
        ...
        ax.add_patch(plt.Rectangle((x_start - 10, y_start - background_height + 10), 
