...
def preprocess_image(image: np.ndarray, enhance_contrast: bool = True) -> tuple[np.ndarray, np.ndarray]:
...
def compute_foreground_background_masks(gray_image: np.ndarray, 
                                        background_thresh: int = 60,
                                        min_physical_length_um: float = 2.0) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    ...
    bg_clean, bg_removed = remove_small_objects_by_size(background_mask_raw, min_physical_length_um, return_removed_mask=True)
    fg_clean, fg_removed = remove_small_objects_by_size(foreground_mask_raw, min_physical_length_um, return_removed_mask=True)
...
def remove_small_objects_by_size(binary_mask: np.ndarray, 
                                  min_physical_length_um: float,
                                  return_removed_mask: bool = False) -> tuple[np.ndarray, np.ndarray] | np.ndarray:
    """
    ...
    """
    px_per_micron = scale_bar_config.SCALE_BAR_PIXELS / scale_bar_config.SCALE_BAR_MICRONS
...
def overlay_removed_regions(gray_image: np.ndarray, removed_mask: np.ndarray, color=(0, 0, 255)) -> np.ndarray:
    """
