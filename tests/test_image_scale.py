...
class TestRealScaleBar(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if not os.path.exists(self.image_path):
            self.skipTest(
                "Real scale bar image not found. Please add it to tests/data/"
            )
    ...
    def test_real_black_scale_bar_length(self):
        ...
        h, w = img_gray.shape
        cropped = img_gray[
            int(h * (1 - self.scale_region_fraction)) :,
            int(w * (1 - self.scale_region_fraction)) :,
        ]
        ...
        cv2.rectangle(
            cropped_color, (x_bg, y_bg), (x_bg + w_bg, y_bg + h_bg), (0, 255, 255), 2
        )
        ...
        for contour in contours:
            ...
            print(
                f"  -> Inside white box: w={w_bar}, h={h_bar}, aspect_ratio={aspect_ratio:.2f}, length={length}"
            )
            if aspect_ratio > 1.5:
        ...
        best_contour, actual_scale_length, (x_rel, y_rel, w_bar, h_bar) = max(
            candidate_lengths, key=lambda x: x[1]
        )
        ...
        cv2.rectangle(
            cropped_color,
            (x_abs, y_abs),
            (x_abs + w_bar, y_abs + h_bar),
            (0, 0, 255),
            2,
        )

        print(
            f"Detected black scale bar length (inside white background): {actual_scale_length} pixels"
        )
