...
class TestWatershedSegmentation(unittest.TestCase):
    ...
    def test_apply_watershed_runs(self):
        ...
        self.assertEqual(
            result.shape, self.image.shape, "Output shape should match original image"
        )
        self.assertEqual(
            result.dtype, np.uint8, "Output image should be in uint8 format"
        )
    ...
    def test_apply_watershed_adds_boundaries(self):
        ...
        self.assertGreater(
            green_pixel_count, 0, "Watershed boundaries should be marked in green"
        )
